"""Thin wrapper around the Gemini API, mirroring app/services/claude_client.py
exactly -- same two functions, same request/response shape, same structured-
output contract (app/services/ai_schemas.py). Selected instead of Claude when
AI_PROVIDER=gemini (see app/services/ai_client.py); the rest of the app never
    ...
    if _client is None:
        if not settings.GEMINI_API_KEY:
            raise GeminiClientError(
                "GEMINI_API_KEY is not configured; cannot run AI-assisted analysis."
            )
...
def check_contradiction(
    ...
    judgment -- same prompt and schema as the Claude path."""
    client = _get_client()
...
def answer_chat_question(context_block: str, history_block: str, question: str) -> ChatAnswer:
    ...
    clause context -- same prompt and schema as the Claude path. Does no
...
def generate_fallback_language(
    ...
    prompt and schema as the Claude path."""
    client = _get_client()
