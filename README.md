...
**Technologies used.** `networkx`, rendered client-side with **React Flow** + **dagre** for automatic node layout.

**Layout, specifically.** Node positions come from two *separate* `dagre` auto-layout passes — one over the MSA's own internal cross-references, one over the SOW's — stacked with the MSA band on top and the SOW band below it, rather than a single shared layout pass. An earlier version ran one dagre pass over the whole graph at once, which let cross-document edges freely interleave MSA and SOW nodes into whatever rank the edge structure implied — structurally defensible, but it read as visually inconsistent rather than as two distinct documents. Cross-document reference edges (the ones that actually matter for spotting a governing-document conflict) still render correctly afterward, as the longer lines connecting the two stacked bands. A faint, dashed-border region box (label in the corner) frames each band so the MSA/SOW split stays visually obvious even without forcing every node into strict per-clause columns.

**Interaction.** Clicking a node opens the full clause text with its section-hierarchy breadcrumb. Selecting a `circular_reference` / `override_conflict` risk flag (see [feature 5](#5-circular-reference--override-conflict-detection)) and switching to this tab highlights that *specific* cycle's nodes and edges distinctly from any other unrelated flagged cycles elsewhere in the graph, rather than marking every flagged node identically regardless of which loop it belongs to. Three checkboxes ("references", "overrides", "flagged / cycle edges") let you declutter the view down to just the edge types you care about — e.g. hide plain references and overrides, keep only cycle edges, to see nothing but the risky loops.

**Benefits.** Interactive, zoomable, clickable — the graph is the single visual anchor for every other feature's findings, and the layout is computed from actual edge structure rather than a fixed grid, so it stays readable as the number of cross-references grows.

**Possible improvements.** The full cross-document graph (both MSA and SOW together) is still recomputed synchronously when a workspace is opened, not continuously pushed — see [Roadmap](#-future-enhancements--roadmap). (Single-document circular-reference detection specifically *is* real-time now, at upload — see [feature 5](#5-circular-reference--override-conflict-detection).) No persisted graph diffing across re-uploads of an amended contract version yet. Very sparse documents (few actual cross-references relative to clause count) still produce a wide, mostly-flat layout within each band, since `dagre` ranks by connectivity and most independent clauses have none.
...
| Backend process exits unexpectedly (no Python traceback, requests start failing with `502`) | **Fixed, in two places.** This was a genuine concurrency bug, not a resource limit: FastAPI runs sync endpoints in a thread pool, and neither the local LLM's llama.cpp context nor the Chat with Contract embedding model's construction/`.encode()` calls were safe for concurrent access from multiple threads at once (concurrent requests reproducibly crashed the whole process in both cases). `app/services/local_llm_client.py` and `app/rag/embedder.py` (plus `app/services/chat_service.py`'s index cache) now each serialize their model loading and inference/encode calls through their own `threading.Lock()` — see `test_local_llm_client.py`, `test_embedder_concurrency.py`, and `test_chat_service_concurrency.py` for the regression tests. If you still see this after updating, it's worth filing as a new issue rather than assuming it's the same bug. |
| Chat with Contract crashes the backend even on a single, non-concurrent request | Likely genuine memory exhaustion, not a bug — verified directly: the crash reproduced identically with a single request when free RAM was ~2.5GB, then succeeded once free RAM was raised to ~4.5GB by closing other applications. The embedding model needs roughly 2-3x its ~1.3GB file size in free memory during loading. Close other memory-heavy applications and retry — see the RAM prerequisite in [Installation](#-installation). |
...
- **Visualization.** Rendered client-side with **React Flow**, laid out via **dagre** (two separate passes, MSA stacked above SOW — see [feature 4](#4-clause-dependency-graph-the-digital-twin) for why) — pannable, zoomable, clickable nodes that open the full clause text with its breadcrumb; edges are color/style-differentiated by `kind`, with a selected cycle highlighted distinctly and edge-type checkboxes to declutter the view.
...
| **Concurrent inference requests** | Serialized through dedicated `threading.Lock()`s in both `local_llm_client.py` (the judgment/generation model) and `app/rag/embedder.py` + `app/services/chat_service.py` (the Chat with Contract embedding model and its index cache) — see [Troubleshooting](#troubleshooting). Neither the llama.cpp context nor the embedding model's construction/encode calls are safe for concurrent access from multiple threads, so a second request arriving while one is already running waits for the first to finish rather than running in parallel. For a single-instance local deployment this is the correct tradeoff (safety over throughput); a multi-instance deployment behind a load balancer would parallelize across processes instead. |
